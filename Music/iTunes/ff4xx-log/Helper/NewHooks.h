...
struct Vars_t
{
    ...
    bool WallHack = false;
    bool WallGlow = false;
    bool WallhackGlow = false; // Kept for legacy if needed
    bool Bypass = true;
    bool Ghost = false;
...
void ProcessAimbot() {
    ...
    if (Vars.AutoAim) shouldAim = true; // تلقائي دائمًا
    if (Vars.AimFire && IsFiring) shouldAim = true; // فقط عند الإطلاق
    if (Vars.AimScope && IsScopeOn) shouldAim = true; // فقط عند السكوب
    if (Vars.FireScope && IsFiring && IsScopeOn) shouldAim = true; // الاثنين معًا
    if (Vars.AimKillFast) shouldAim = true; // دائمًا للكيل السريع
    if (Vars.AimKillFire && IsFiring) shouldAim = true; // كيل عند الإطلاق
    ...
    if (shouldAim && (!Vars.VisibleCheck || tanghinh::isVisible(closestEnemy))) {
        ...
        if (Vars.AimKill || Vars.AimKillFast) {
            ...
            if (distToCenter > 85.0f) isFacingTarget = false; // تقليل المسافة لدقة أعلى
        }
            ...
            if (w2sc) {
                ...
                if (Vars.Name) {
                    ...
                    if (pname) {
                        ...
                        draw_list->AddText(ImVec2(rect.GetCenter().x - 10, rect.Min.y - 15), ImGui::ColorConvertFloat4ToU32(nameColor), name.c_str());
                    }
                }
                if (Vars.Distance) {
                    std::string dist = fmt::format("{}M", (int)distance);
                    draw_list->AddText(ImVec2(rect.Max.x + 2, rect.Min.y), ImGui::ColorConvertFloat4ToU32(distanceColor), dist.c_str());
                }
...
void ToggleWallHack(bool active) {
    JRMemoryEngine engine(mach_task_self());
    AddrRange range = {0x100000000, 0x160000000};
    float searchVal = active ? 1.5f : 965.0f;
    float replaceVal = active ? 965.0f : 1.5f;
...
void ToggleWallGlow(bool active) {
    JRMemoryEngine engine(mach_task_self());
    AddrRange range = {0x100000000, 0x160000000};
    float searchVal = active ? 1.22f : 965.0f;
    float replaceVal = active ? 965.0f : 1.22f;
