...
TOKEN = os.getenv("TELEGRAM_TOKEN_BOT") or os.getenv("TELEGRAM_BOT_TOKEN") or "8308447806:AAGpj-E-_1jOTvA7vk9Nq1zKH48sC3YCjK8"
...
BINANCE_PAY_CERT_SN = os.getenv("BINANCE_PAY_CERT_SN", "")
BINANCE_PAY_CURRENCY = os.getenv("BINANCE_PAY_CURRENCY", "USDT")
ONCHAIN_BSC_API_BASE = os.getenv("ONCHAIN_BSC_API_BASE", "https://api.bscscan.com/api")
ONCHAIN_BSC_API_KEY = os.getenv("ONCHAIN_BSC_API_KEY", "")
ONCHAIN_BSC_CHAIN_ID = os.getenv("ONCHAIN_BSC_CHAIN_ID", "56")
ONCHAIN_BSC_WALLET_ADDRESS = (os.getenv("ONCHAIN_BSC_WALLET_ADDRESS", "") or "").lower()
ONCHAIN_BSC_TOKEN_CONTRACT = (os.getenv("ONCHAIN_BSC_TOKEN_CONTRACT", "0x55d398326f99059fF775485246999027B3197955") or "").lower()
ONCHAIN_BSC_TOKEN_SYMBOL = os.getenv("ONCHAIN_BSC_TOKEN_SYMBOL", "USDT")
...
def _binance_is_enabled():
  return bool(BINANCE_PAY_API_KEY and BINANCE_PAY_SECRET and BINANCE_PAY_CERT_SN)


def _binance_headers(body: str):
  timestamp = str(int(time.time() * 1000))
  nonce = uuid.uuid4().hex[:32]
  payload = f"{timestamp}\n{nonce}\n{body}\n"
  signature = hmac.new(
      BINANCE_PAY_SECRET.encode("utf-8"),
  ...
  return {
      ...
      "BinancePay-Certificate-SN": BINANCE_PAY_CERT_SN,
      "BinancePay-Signature": signature,
      "BinancePay-Api-Key": BINANCE_PAY_API_KEY,
  }


def _binance_post(path: str, payload: dict):
  if not _binance_is_enabled():
      return False, {"error": "Binance Pay is not configured"}

  url = BINANCE_PAY_BASE_URL.rstrip("/") + path
  body = json.dumps(payload, separators=(",", ":"), ensure_ascii=False)
  headers = _binance_headers(body)

  try:
      if requests:
          resp = requests.post(url, data=body.encode("utf-8"), headers=headers, timeout=25)
          try:
              data = resp.json()
          except Exception:
...
def create_binance_order(user_id: str, amount: float):
  merchant_trade_no = f"BAL-{user_id}-{int(time.time())}-{uuid.uuid4().hex[:6]}"
  payload = {
      "merchantTradeNo": merchant_trade_no,
      "orderAmount": float(amount),
      "currency": BINANCE_PAY_CURRENCY,
      "goods": {
          "goodsType": "01",
          "goodsCategory": "D000",
          "referenceGoodsId": "wallet-topup",
          "goodsName": "Wallet top-up",
          "goodsDetail": f"Telegram user {user_id} wallet top-up"
      }
  }
  ok, resp = _binance_post("/binancepay/openapi/v2/order", payload)
  if not ok or not isinstance(resp, dict):
      return False, merchant_trade_no, None, resp
  if resp.get("status") != "SUCCESS" and resp.get("code") != "000000":
      return False, merchant_trade_no, None, resp
  data = resp.get("data", {}) or {}
  pay_url = data.get("checkoutUrl") or data.get("universalUrl") or data.get("deeplink")
  return bool(pay_url), merchant_trade_no, pay_url, resp


def query_binance_order(merchant_trade_no: str):
  payload = {"merchantTradeNo": merchant_trade_no}
  ok, resp = _binance_post("/binancepay/openapi/v2/order/query", payload)
  if not ok or not isinstance(resp, dict):
...
def _is_paid_binance_status(status: str):
  return status in {"PAID", "SUCCESS", "COMPLETED"}


def _finalize_binance_credit(merchant_trade_no: str, status: str):
  data = load_data()
  pending = data.setdefault("pending_payments", {}).get(merchant_trade_no)
  if not pending:
      return False, None, 0
  if pending.get("credited"):
      return False, pending.get("user_id"), float(pending.get("amount", 0))

  user_id = str(pending.get("user_id"))
  amount = float(pending.get("amount", 0))
  data.setdefault("balances", {})[user_id] = float(data.get("balances", {}).get(user_id, 0)) + amount
  pending["credited"] = True
  pending["status"] = status
  pending["credited_at"] = int(time.time())
  save_data(data)
  return True, user_id, amount


def _binance_watch_order(merchant_trade_no: str):
  for _ in range(60):
      data = load_data()
      pending = data.get("pending_payments", {}).get(merchant_trade_no)
      if not pending or pending.get("credited"):
          return

      ok, status, _ = query_binance_order(merchant_trade_no)
      if ok and _is_paid_binance_status(status):
          changed, user_id, amount = _finalize_binance_credit(merchant_trade_no, status)
          if changed:
              try:
                  bot = telegram.Bot(token=TOKEN)
                  bot.send_message(chat_id=int(user_id), text=f"✅ Binance payment received. ${amount} has been added to your balance." + SIGNATURE)
              except Exception:
                  pass
          return

      if ok and status in {"EXPIRED", "CANCELED", "FAIL", "FAILED", "ERROR"}:
          data = load_data()
          if merchant_trade_no in data.get("pending_payments", {}):
              data["pending_payments"][merchant_trade_no]["status"] = status
              save_data(data)
          return
      time.sleep(10)


def _onchain_is_enabled():
  return bool(ONCHAIN_BSC_API_KEY and ONCHAIN_BSC_WALLET_ADDRESS)


def _onchain_fetch_token_txs(limit: int = 50):
  if not _onchain_is_enabled():
      return False, []
  params = {
      "module": "account",
      "action": "tokentx",
      "address": ONCHAIN_BSC_WALLET_ADDRESS,
      "contractaddress": ONCHAIN_BSC_TOKEN_CONTRACT,
      "page": 1,
      "offset": max(1, min(int(limit), 100)),
      "sort": "desc",
      "apikey": ONCHAIN_BSC_API_KEY,
  }
  try:
      if requests:
          resp = requests.get(ONCHAIN_BSC_API_BASE, params=params, timeout=20)
          data = resp.json()
      else:
          q = urllib.parse.urlencode(params)
          with urllib.request.urlopen(f"{ONCHAIN_BSC_API_BASE}?{q}", timeout=20) as response:
...
def _create_onchain_order(user_id: str, base_amount: float):
  suffix = ((int(uuid.uuid4().hex[:4], 16) % 90) + 10) / 1000.0
  pay_amount = round(float(base_amount) + suffix, 3)
  order_id = f"ONC-{user_id}-{int(time.time())}-{uuid.uuid4().hex[:6]}"
  return order_id, pay_amount


def _find_matching_onchain_tx(payment: dict, used_hashes: set):
  ok, txs = _onchain_fetch_token_txs(limit=100)
  if not ok:
      return False, "API_ERROR", None

  expected = float(payment.get("amount", 0))
  created_at = int(payment.get("created_at", 0))
  for tx in txs:
      try:
          ...
          if to_addr != ONCHAIN_BSC_WALLET_ADDRESS:
              continue
          ...
          if abs(amount - expected) > 0.000001:
              continue
          confirmations = int(tx.get("confirmations") or 0)
          if confirmations < 1:
              return True, "PENDING_CONFIRMATION", tx_hash
          return True, "PAID", tx_hash
      except Exception:
          continue
  return True, "PENDING", None


def _finalize_onchain_credit(order_id: str, tx_hash: str):
  data = load_data()
  payment = data.setdefault("pending_payments", {}).get(order_id)
  if not payment:
      return False, None, 0
  if payment.get("credited"):
      return False, payment.get("user_id"), float(payment.get("amount", 0))

  user_id = str(payment.get("user_id"))
  amount = float(payment.get("amount", 0))
  data.setdefault("balances", {})[user_id] = float(data.get("balances", {}).get(user_id, 0)) + amount
  if payment.get("credit_seller"):
      sellers = data.setdefault("sellers", {})
      if user_id in sellers:
          sellers[user_id]["balance"] = float(sellers[user_id].get("balance", 0)) + amount
  payment["credited"] = True
  payment["status"] = "PAID"
  payment["tx_hash"] = tx_hash
  payment["credited_at"] = int(time.time())
  used = set(data.get("used_payment_txs", []))
  used.add(tx_hash)
  data["used_payment_txs"] = list(used)
  save_data(data)
  return True, user_id, amount


def _onchain_watch_order(order_id: str):
  for _ in range(90):
      data = load_data()
      payment = data.get("pending_payments", {}).get(order_id)
      if not payment or payment.get("credited"):
          return
      used = set(data.get("used_payment_txs", []))
      ok, status, tx_hash = _find_matching_onchain_tx(payment, used)
      if ok and status == "PAID" and tx_hash:
          changed, user_id, amount = _finalize_onchain_credit(order_id, tx_hash)
          if changed:
              try:
                  bot = telegram.Bot(token=TOKEN)
                  bot.send_message(chat_id=int(user_id), text=f"✅ On-chain payment received. ${amount} added to your balance." + SIGNATURE)
              except Exception:
                  pass
          return
      if ok and status == "PENDING_CONFIRMATION":
          data = load_data()
          if order_id in data.get("pending_payments", {}):
              data["pending_payments"][order_id]["status"] = "PENDING_CONFIRMATION"
              save_data(data)
      time.sleep(10)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  menu_items = [button_texts["buy"][lang], button_texts["balance"][lang], button_texts["deposit"][lang], button_texts["activity"][lang]]
  if uid in admins:
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  if data == "back_to_start":
      ...
      menu_items = [
          "🛍️ BUY KEYS" if lang == "en" else "🛍️ شراء مفاتيح",
          "💰 MY BALANCE" if lang == "en" else "💰 رصيدي",
          "💳 DEPOSIT" if lang == "en" else "💳 إيداع",
          "📊 Activity" if lang == "en" else "📊 النشاط"
      ]
      if uid in admins:
  ...
  if data == "show_balance":
      ...
      keyboard = [
          [InlineKeyboardButton("💳 Deposit Binance", callback_data="binance_deposit_menu")],
          [InlineKeyboardButton("⬅️ Back", callback_data="back_to_start")]
      ]
      await query.edit_message_text(f"Your balance: ${bal}" + SIGNATURE, reply_markup=InlineKeyboardMarkup(keyboard))
      return

  if data == "binance_deposit_menu":
      if not _binance_is_enabled():
          await query.edit_message_text(
              "⚠️ Binance Pay is not configured by admin yet." + SIGNATURE,
              reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back", callback_data="show_balance")]])
          )
          return
      keyboard = [
          [InlineKeyboardButton("$5", callback_data="binance_deposit_amount:5"), InlineKeyboardButton("$10", callback_data="binance_deposit_amount:10")],
          [InlineKeyboardButton("$20", callback_data="binance_deposit_amount:20"), InlineKeyboardButton("$50", callback_data="binance_deposit_amount:50")],
          [InlineKeyboardButton("⬅️ Back", callback_data="show_balance")]
      ]
      await query.edit_message_text(
          "Choose deposit amount (USDT):" + SIGNATURE,
          reply_markup=InlineKeyboardMarkup(keyboard)
      )
      return

  if data and data.startswith("binance_deposit_amount:"):
      if not _binance_is_enabled():
          await query.edit_message_text(
              "⚠️ Binance Pay is not configured by admin yet." + SIGNATURE,
              reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back", callback_data="show_balance")]])
      ...
      try:
          ...
          await query.edit_message_text(
              ...
              reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back", callback_data="binance_deposit_menu")]])
          )
          return

      ok, merchant_trade_no, pay_url, api_resp = create_binance_order(uid, amount)
      if not ok or not pay_url:
          err = "Unknown error"
          if isinstance(api_resp, dict):
              err = api_resp.get("error") or api_resp.get("msg") or str(api_resp)
          await query.edit_message_text(
              f"❌ Failed to create Binance payment.\n{err}" + SIGNATURE,
              reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back", callback_data="binance_deposit_menu")]])
          )
          return

      DATA.setdefault("pending_payments", {})[merchant_trade_no] = {
          "user_id": uid,
          "amount": amount,
          "provider": "binance",
          "status": "PENDING",
      ...
      Thread(target=_binance_watch_order, args=(merchant_trade_no,), daemon=True).start()

      keyboard = [
          [InlineKeyboardButton("💳 Pay Now", url=pay_url)],
          [InlineKeyboardButton("🔄 Check Payment", callback_data=f"binance_check_payment:{merchant_trade_no}")],
          [InlineKeyboardButton("⬅️ Back", callback_data="show_balance")]
      ]
      await query.edit_message_text(
          f"Binance payment created.\nAmount: ${amount}\nOrder: {merchant_trade_no}\n\nOpen Pay Now to complete payment. Balance is added automatically after payment." + SIGNATURE,
          reply_markup=InlineKeyboardMarkup(keyboard)
      )
      return

  if data and data.startswith("binance_check_payment:"):
      merchant_trade_no = data.split(":", 1)[1]
      payment = DATA.get("pending_payments", {}).get(merchant_trade_no)
      if not payment:
          await query.edit_message_text(
              "⚠️ Payment record not found." + SIGNATURE,
              reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back", callback_data="show_balance")]])
          )
          return
      if str(payment.get("user_id")) != uid:
          await query.edit_message_text(
              "❌ This payment does not belong to your account." + SIGNATURE,
              reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back", callback_data="show_balance")]])
          )
          return
      ...
      if _is_paid_binance_status(str(status).upper()):
          changed, _, amount = _finalize_binance_credit(merchant_trade_no, str(status).upper())
          refreshed = load_data()
          bal = refreshed.get("balances", {}).get(uid, 0)
          text = f"✅ Payment confirmed. ${amount} added.\nYour balance: ${bal}" if changed else f"✅ Payment already credited.\nYour balance: ${bal}"
          await query.edit_message_text(
              text + SIGNATURE,
              reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back", callback_data="show_balance")]])
          )
      ...
      await query.edit_message_text(
          f"⏳ Payment status: {status or 'PENDING'}\nPlease complete payment and try again in a few seconds." + SIGNATURE,
          reply_markup=InlineKeyboardMarkup([
              [InlineKeyboardButton("🔄 Check Again", callback_data=f"binance_check_payment:{merchant_trade_no}")],
              [InlineKeyboardButton("⬅️ Back", callback_data="show_balance")]
          ])
      )
      return
  ...
  if data and data.startswith("pay:"):
      ...
      for k in bonus_keys_list:
          sale_entry = {"user": user_id, "product": product, "duration": days, "price": 0, "key": k, "note": "Bonus Key"}
          if seller_id:
              sale_entry["seller_id"] = seller_id
              sale_entry["seller_name"] = seller_name
          DATA.setdefault("sales_log", []).append(sale_entry)
      ...
      await query.edit_message_text(
          f"✅ Purchase successful!\n\nYour keys:\n{keys_str}{bonus_keys_msg}\n\n📁 Use Get Files to download product updates." + bonus_msg + SIGNATURE,
          parse_mode="Markdown",
          reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Back", callback_data="buy_menu")]])
      )
      return
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  if text == button_texts["deposit"][lang]:
      user_id = str(update.message.from_user.id)
      data = load_data()
      msg = {
          "en": "💳 Choose deposit method:",
          "ar": "💳 اختر طريقة الإيداع:",
          "fr": "💳 Choisissez la méthode de dépôt:",
          "es": "💳 Elija el método de depósito:"
      }.get(lang, "💳 Choose deposit method:")
      
      buttons = []
      if _binance_is_enabled():
          buttons.append([InlineKeyboardButton("💳 Deposit Binance", callback_data="binance_deposit_menu")])
      if _onchain_is_enabled():
          buttons.append([InlineKeyboardButton(f"⛓️ Deposit {ONCHAIN_BSC_TOKEN_SYMBOL} (BSC)", callback_data="onchain_deposit_menu")])
      if uid in data.get("sellers", {}) and _onchain_is_enabled():
          buttons.append([InlineKeyboardButton(f"🏷️ Seller Deposit {ONCHAIN_BSC_TOKEN_SYMBOL}", callback_data="seller_onchain_deposit_menu")])
          
      if buttons:
          kb = InlineKeyboardMarkup(buttons)
          await update.message.reply_text(msg + SIGNATURE, reply_markup=kb)
      else:
          await update.message.reply_text("❌ No deposit methods available." + SIGNATURE)
      return
...
def main():
    if not TOKEN:
        print("Error: Set TELEGRAM_TOKEN_BOT (or TELEGRAM_BOT_TOKEN) in environment variables.")
        return
